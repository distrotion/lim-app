...
class P41XRFREQTABLE extends StatefulWidget {
  P41XRFREQTABLE({Key? key, this.sardata}) : super(key: key);
  List<dataset>? sardata;

  @override
  State<P41XRFREQTABLE> createState() => _P41XRFREQTABLEState();
}

class _P41XRFREQTABLEState extends State<P41XRFREQTABLE> {
  @override
  void initState() {
    super.initState();
  }
            ...
            child: Row(
              ...
              children: [
                ComInputText(
                  ...
                  height: 40,
                  width: 400,
                  isContr: P41XRFREQVAR.iscontrol,
                  fnContr: (input) {
                    P41XRFREQVAR.iscontrol = input;
                  },
                  sValue: P41XRFREQVAR.SEARCH,
                  returnfunc: (String s) {
                    setState(() {
                      P41XRFREQVAR.SEARCH = s;
                    });
                  },
                ),
                const Spacer(),
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < _data.length; i++) {
      if (_data[i]
                  .f01
                  .toUpperCase()
                  .contains(P41XRFREQVAR.SEARCH.toUpperCase()) ||
              _data[i]
                  .f02
                  .toUpperCase()
                  .contains(P41XRFREQVAR.SEARCH.toUpperCase()) ||
              ...
              _data[i]
                  ...
                  .toUpperCase()
                  .contains(P41XRFREQVAR.SEARCH.toUpperCase())
          // _data[i].f05.contains(P41XRFREQVAR.SEARCH)
          ) {
          ...
          if (CP == "XRF") {
            final response = Dio().post(
              ...
              if (value.statusCode == 200) {
                ...
                // print('>>>>${databuXRF['REQNO']}');
                if (databuXRF['REQNO'] != null) {
                  if (databuXRF['REQNO'].toString() == '') {
                    final response = Dio().post(
...
//   MainBodyContext.read<ChangePage_Bloc>().add(ChangePage_nodrower());
// } else if (CP == 'XRF') {
//   P3BALANCEBODY01XRFVAR.ReqNo = PO;
//   P3BALANCEBODY01XRFVAR.TYPE = CP;
//   CuPage = Page3();
//   MainBodyContext.read<ChangePage_Bloc>().add(ChangePage_nodrower());
// } else {
//   //
