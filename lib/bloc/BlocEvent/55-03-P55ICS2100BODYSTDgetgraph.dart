...
abstract class P55ICS2100BODYSTDgetgraph_Event {}

class P55ICS2100BODYSTDgetgraph_get extends P55ICS2100BODYSTDgetgraph_Event {}

class P55ICS2100BODYSTDgetgraph_flush extends P55ICS2100BODYSTDgetgraph_Event {}

class P55ICS2100BODYSTDgetgraph_Bloc extends Bloc<P55ICS2100BODYSTDgetgraph_Event, List<HistoryChartModel>> {
  P55ICS2100BODYSTDgetgraph_Bloc() : super([]) {
    on<P55ICS2100BODYSTDgetgraph_get>((event, emit) {
      return _P55ICS2100BODYSTDgetgraph_get([], emit);
    });
    on<P55ICS2100BODYSTDgetgraph_flush>((event, emit) {
      return _P55ICS2100BODYSTDgetgraph_flush([], emit);
    });
  }
  Future<void> _P55ICS2100BODYSTDgetgraph_get(
      List<HistoryChartModel> toAdd, Emitter<List<HistoryChartModel>> emit) async {
    List<HistoryChartModel> output = [];
    print("----------------------------");
    print(P55ICS2100BODYSTDVAR.UID);
    print(P55ICS2100BODYSTDVAR.ItemName);
    print("----------------------------");
    final response = await Dio().post(
      '${serverSARMAIN}/Widget_SearcHistoryChartData2',
      data: {
        'itemID': P55ICS2100BODYSTDVAR.UID,
        'itemName': P55ICS2100BODYSTDVAR.ItemName,
        'section': "TTC",
        // 'itemID': '967644',
        // 'itemName': 'XRF',
        // 'section': "TTC",
  ...
  Future<void> _P55ICS2100BODYSTDgetgraph_flush(
      List<HistoryChartModel> toAdd, Emitter<List<HistoryChartModel>> emit) async {
    List<HistoryChartModel> output = [];
    emit(output);
  }
...
List<HistoryChartModel> historyChartModelFromJson(String str) =>
    List<HistoryChartModel>.from(json.decode(str).map((x) => HistoryChartModel.fromJson(x)));
