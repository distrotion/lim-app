...
abstract class P57UVBODYSTDgetgraph_Event {}

class P57UVBODYSTDgetgraph_get extends P57UVBODYSTDgetgraph_Event {}

class P57UVBODYSTDgetgraph_flush extends P57UVBODYSTDgetgraph_Event {}

class P57UVBODYSTDgetgraph_Bloc
    extends Bloc<P57UVBODYSTDgetgraph_Event, List<HistoryChartModel>> {
  P57UVBODYSTDgetgraph_Bloc() : super([]) {
    on<P57UVBODYSTDgetgraph_get>((event, emit) {
      return _P57UVBODYSTDgetgraph_get([], emit);
    });
    on<P57UVBODYSTDgetgraph_flush>((event, emit) {
      return _P57UVBODYSTDgetgraph_flush([], emit);
    });
  }
  Future<void> _P57UVBODYSTDgetgraph_get(List<HistoryChartModel> toAdd,
      Emitter<List<HistoryChartModel>> emit) async {
    List<HistoryChartModel> output = [];
    print("----------------------------");
    print(P57UVBODYSTDVAR.UID);
    print(P57UVBODYSTDVAR.ItemName);
    print("----------------------------");
    final response = await Dio().post(
      '${serverSARMAIN}/Widget_SearcHistoryChartData2',
      data: {
        'itemID': P57UVBODYSTDVAR.UID,
        'itemName': P57UVBODYSTDVAR.ItemName,
        'section': "TTC",
        // 'itemID': '967644',
        // 'itemName': 'UV',
        // 'section': "TTC",
  ...
  Future<void> _P57UVBODYSTDgetgraph_flush(List<HistoryChartModel> toAdd,
      Emitter<List<HistoryChartModel>> emit) async {
    List<HistoryChartModel> output = [];
    emit(output);
  }
